...
#ensure output directory exists for output folder and template folder
mkdir(args.output_dir)
mkdir(args.template_dir)
template_dir = args.template_dir

#for UI template dropdown selector
def get_template_files(template_dir):  
    return [f for f in os.listdir(template_dir) if f.endswith('.pkl')]
                ...
                with gr.TabItem("🎞️ Driving Video") as tab_image:
                    ...
                    with gr.Accordion(open=False, label="Driving Examples"):
                        gr.Examples(
                            ...
                            cache_examples=False,
                            label="Driving Examples"
                        )
                        
                with gr.TabItem("🏎️ Driving Templates") as tab_video:
                    with gr.Accordion(open=True, label="Driving Templates"):
                        template_dropdown = gr.Dropdown(
                            choices=["None"] + get_template_files(args.template_dir),
                            label="Select a template",
                            value="None",
                            info="Choose a pre-made template instead of uploading a driving video.  Note that video input will take priority if video input is populated" )
 
        
                        refresh_button = gr.Button("Refresh Templates", variant="secondary")
            
                        def refresh_templates():
                            return gr.Dropdown(choices=["None"] + get_template_files(args.template_dir), value="None")

                            
#gradio cropping section
    ...
    with gr.Row():
        ...
        with gr.Column():      
                with gr.Accordion(open=False, label="Cropping Options for Driving Video"):
                    with gr.Row():
                        flag_crop_driving_video_input = gr.Checkbox(value=False, label="crop (driving)")
...
#gradio animate, clear, output folder buttons
